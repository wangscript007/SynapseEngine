...
namespace AssimpConverter
{
	...
	static void AddMeshesAndBones(const aiScene* a_pScene, SkeletalModel& a_OutModel)
	{
		...
		for (unsigned int i = 0; i < a_pScene->mNumMeshes; ++i)
		{
			...
			AnimMesh.pUV = new glm::vec2[AnimMesh.NumVertices];
			AnimMesh.pVertices = new glm::vec3[AnimMesh.NumVertices];
			AnimMesh.pBiNormals = new glm::vec3[AnimMesh.NumVertices];
			AnimMesh.pTangents = new glm::vec3[AnimMesh.NumVertices];
			...
			AnimMesh.pTransformedBiNormals = new glm::vec3[AnimMesh.NumVertices];
			AnimMesh.pTransformedTangents = new glm::vec3[AnimMesh.NumVertices];
			...
			memcpy(AnimMesh.pBiNormals, pMesh->mBitangents, AnimMesh.NumVertices * sizeof(aiVector3D));
			memcpy(AnimMesh.pTangents, pMesh->mTangents, AnimMesh.NumVertices * sizeof(aiVector3D));

			for (int k = 0; k < AnimMesh.NumVertices; k++) {

				auto uv = pMesh->mTextureCoords[0][k];
				glm::vec2 auv = glm::vec2(uv.x, uv.y);
				AnimMesh.pUV[k] = auv;
			...
			for (unsigned int i = 0; i < pMesh->mNumBones; ++i)
			{
				...
				for (unsigned int i = 0; i < Bone.NumWeights; ++i)
				{
					...
					//printf("VID:%d\n", Bone.pWeights[i].VertexID);
					if (Bone.pWeights[i].VertexID > 100000)
					{
						int vv = 5;
					}
